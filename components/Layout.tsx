  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-dark-900 overflow-hidden transition-colors duration-300">
      ...
      `}>
        ...
        `}>
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <div className={`transition-opacity duration-200 ${isDesktopSidebarCollapsed ? 'hidden opacity-0 w-0' : 'block opacity-100'}`}>
                  ...
                  <button
                    ...
                    className={`
                      ...
                      ${active 
                        ...
                        : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-dark-800 hover:text-gray-900 dark:hover:text-gray-200'}
                      ${isDesktopSidebarCollapsed ? 'justify-center' : ''}
                    `}
                    title={isDesktopSidebarCollapsed ? item.label : ''}
                  >
                    {active && !isDesktopSidebarCollapsed && (
                      <span className="absolute right-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-primary-600 rounded-l-full"></span>
                    )}
                    <div className="relative">
                      <Icon size={22} className={`min-w-[22px] ${active ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400 dark:text-gray-500 group-hover:text-primary-600 dark:group-hover:text-primary-400'}`} />
                    ...
                    <span className={`whitespace-nowrap transition-all duration-300 ${isDesktopSidebarCollapsed ? 'hidden w-0 opacity-0' : 'block w-auto opacity-100'}`}>
                      {item.label}
                    </span>
                  </button>
                </li>
        ...
        {/* User Profile Footer */}
        <div className="p-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-dark-900/50">
          <div className={`flex items-center gap-3 mb-4 ${isDesktopSidebarCollapsed ? 'justify-center' : ''}`}>
            <div className="relative">
               ...
               <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border-2 border-white dark:border-dark-900 rounded-full"></span>
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 transition-all duration-300">
        {/* Header */}
        <header className="bg-white dark:bg-dark-950 border-b border-gray-200 dark:border-gray-800 h-20 flex items-center justify-between px-4 lg:px-8 z-30 transition-colors duration-300">
          ...
          <div className="flex items-center gap-3 md:gap-5">
                          ...
                          <ul className="py-2 max-h-80 overflow-y-auto custom-scrollbar">
                              {searchResults.map((result) => (
                                  <li key={`${result.type}-${result.id}`}>
                                      <button 
                                          className="w-full text-right px-4 py-3 hover:bg-gray-50 dark:hover:bg-dark-800 flex items-center gap-3 transition-colors"
            ...
            >
               {isDarkMode ? <Sun size={20} className="animate-in spin-in-90" /> : <Moon size={20} className="animate-in spin-in-90" />}
            </button>
            ...
            <div className="relative" ref={notificationRef}>
              ...
              >
                ...
                {activeNotifications > 0 && (
                  <span className="absolute top-2 right-2.5 w-2 h-2 bg-red-500 rounded-full border border-white dark:border-dark-950"></span>
                )}
              </button>

              {showNotifications && (
                <div className="absolute left-0 mt-3 w-80 bg-white dark:bg-dark-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
